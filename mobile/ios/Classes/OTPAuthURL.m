...
static NSString *const kHOTPAuthScheme = @"hotp";
...
+ (OTPAuthURL *)authURLWithURL:(NSURL *)url
  ...
  } else if ([urlScheme isEqualToString:kTOTPAuthScheme]) {
    // Convert totp:// into otpauth://
    authURL = [[[HOTPAuthURL alloc] initWithHOTPURL:url] autorelease];
  }
...
// hotp:// urls are generated by some sites. the otpauth:// scheme
// provides more features so we translate to that. this is consistent with
// how we handle totp:// urls
- (id)initWithHOTPURL:(NSURL *)url {
    NSMutableString *name = nil;
    if ([[url user] length]) {
        name = [NSMutableString stringWithString:[url user]];
